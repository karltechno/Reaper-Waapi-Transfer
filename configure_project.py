...
def copy_akautobahn(wwise_sdk_dir):
    ...
    dest_akautobahn = os.path.join(os.path.dirname(__file__), 'ext', 'AkAutobahn', 'AkAutobahn')
...
def cmake_run(generator, platform, build_dir, cmake_vars):
    path_to_src = os.path.relpath(os.getcwd(), build_dir)
...
def main():
    ...
    argparser.add_argument('-wwise_sdk_root', required=False, help='Path to Wwise sdk, uses environment var by default')
    argparser.add_argument('-cmake_generator', help='CMake Generator.', default='Visual Studio 15 2017')
    argparser.add_argument('-cmake_generator_platform', help='CMake generator platform', default='x64')
    argparser.add_argument('-build_dir', help='Path to generate solution', default='build')

    argparser.add_argument('-reaper32_dir', help='Path to reaper32 directory.', required=False)
    argparser.add_argument('-reaper64_dir', help='Path to reaper64 directory.', required=False)
